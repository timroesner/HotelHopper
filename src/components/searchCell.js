...
class SearchCell extends React.Component {
    ...
    render() {
        ...
        return(
        <div className="z-50 flex mt-1 w-full overflow-hidden border-b py-3 border-grey-light" value={hotel.hotelId} onClick={e => this.goTo(hotelQuery)} style={{ cursor: 'pointer' }} key={hotel.hotelId} >
                  <div className="w-full justify-between flex col-md-6 ">
                    <div className="flex col-md-6">
                    <img className="h-32 w-48 md:h-48 md:w-64 mr-2 rounded" style={{objectFit: "cover"}} alt="of Hotel" src={hotel.imageUrl}/>
                    <div className="justify-between flex-col md:ml-8">  
                      <div  className="font-bold w-full overflow-hidden align-start text-lg md:text-xl ml-2 mb-2">{hotel.title}</div>
                      <div className=" text-grey-dark font-sans align-start pb:2 text-sm md:text-base ml-2 mt-2">{hotel.city}</div>
                      <div className="font-bold align-start mt-2 mb-2 md:mb-4 text-sm md:text-xl ml-2"> 
                        <StarRatings
